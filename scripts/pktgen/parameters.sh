...
if [ ! -d /proc/net/pktgen ]; then
    ...
    modprobe pktgen
    if [ $? -ne 0 ] ; then 
	echo -e "\nERROR: 'modprobe pktgen' failed!\n"
	exit 1
    fi 
    sleep 3
    if [ ! -d /proc/net/pktgen ]; then
	echo -e "\nERROR: /proc/net/pktgen not found after install 'pktgen' kernel modules!\n"
	exit 2
    fi 
else
    echo -e "\nINFO: /proc/net/pktgen found, use kernel pktgen for traffic generation...\n"
fi
